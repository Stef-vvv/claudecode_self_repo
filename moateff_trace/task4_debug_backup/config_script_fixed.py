    ...
    for user_index, folder_name in INDEX_FOLDER_MAP.items():
        ...
        for f in required_files:
            ...
            with open(serial_data_path, 'w') as f:
                # FIXED: use FORWARD order (same as scan_chain.txt).
                # Hardware scan chain: scan_in → H → W → ... → t → PE_config → scan_out
                # First bit loaded = H[7], last bit loaded = opsum_ids[0]
                # Original reversal was a bug — PE bits ended up in parameter registers.
                for bit in full_chain:
                    f.write(f'{bit}\n')
