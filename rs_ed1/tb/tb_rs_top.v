...
module tb_rs_top;
    ...
    initial begin
        ...
        // ====================================
        // Test1: 阵列0验证 — 手算结果
        // ifmap: pad=0, col0..4=[1,2,3,4,5], 其余=0
        // filter: [1,2,3],[4,5,6],[7,8,9]
        // 阵列0数据=[0,1,2,3,4] → 3行累加 → [51,96,141] (已验证)
        // 阵列1数据=[3,4,5,6,7] → [186,231,276] (已验证)
        // ====================================
        $display("=== Test1: Array0 & Array1 non-zero verification ===");
        @(negedge clk);
        ...
        repeat(40) begin
            ...
            if (td) begin
                ...
                if (ar[0*AW+:AW]!=0 && ar[1*AW+:AW]!=0)
                    $display("Array0 NON-ZERO: PASS");
                else begin $display("Array0 ZERO: FAIL"); tp=0; end
                if (av) $display("acc_valid=1: PASS");
                else begin $display("acc_valid=0: FAIL"); tp=0; end
