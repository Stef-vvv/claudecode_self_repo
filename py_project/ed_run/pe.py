...
class PE:
    ...
    def process_one(self):
        ...
        PE内部保持全精度(float), Q8量化在聚合器/写回阶段完成 (匹配硬件: PE用Q16累加, 输出时右移8位).
        """
        ...
        # ---- MAC 计算 (start拍也进入, state刚被置1) ----
        if self.state == 1:
            dot = sum(self.sliding_window[i] * self.filter[i] for i in range(3))
            self.result[self.iteration] = dot  # 全精度
            self.iteration += 1
